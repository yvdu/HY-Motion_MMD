...
def _safe_pipeline_to(pipeline, device) -> None:
    """按子模块迁移，避免 text_encoder 内 meta/offload 权重触发 NotImplementedError。"""
    for name, child in pipeline.named_children():
        try:
            child.to(device)
        except NotImplementedError as e:
            print(f">>> skip .to({device}) for '{name}': {e}")
...
class T2MRuntime:
    ...
    def load(self):
        ...
        if not self.device_ids:
            ...
            _safe_pipeline_to(pipeline, device)
            ...
            for gid in self.device_ids:
                ...
                _safe_pipeline_to(p, torch.device(f"cuda:{gid}"))
