...
class MotionGeneration(torch.nn.Module):
    ...
    def _build_text_encoder(self, allow_llm: bool = True):
        """构建文本编码器（必须含 Qwen），加载失败直接抛错。"""
        cfg = deepcopy(self._text_encoder_cfg)
        if not allow_llm or not cfg.get("llm_type"):
            raise RuntimeError("text_encoder_cfg 未配置 llm_type，无法加载 Qwen 文本编码器")

        print(">>> Loading full text encoder (CLIP + Qwen)…")
        try:
            enc = load_object(self._text_encoder_module, cfg)
        except Exception as e:
            raise RuntimeError(
                f"Qwen 文本编码器加载失败: {e}\n"
                "请确认 ckpts/Qwen3-8B 完整，且内存/显存足够。"
            ) from e

        if not getattr(enc, "has_llm", False):
            raise RuntimeError(
                "Qwen 文本编码器未就绪（has_llm=False）。"
                "请检查 ckpts/Qwen3-8B 权重是否完整。"
            )
        print(">>> Text encoder ready (CLIP + Qwen).")
        return enc
    ...
    def load_in_demo(
        ...
        if build_text_encoder and not self.uncondition_mode:
            self.text_encoder = self._build_text_encoder(allow_llm=True)
            try:
                self.text_encoder.to(get_module_device(self))
            except NotImplementedError:
                # meta / offload 权重不可整体迁移
                pass
    ...
    def encode_text(self, text: Dict[str, List[str]]) -> Dict[str, Tensor]:
        if not hasattr(self, "text_encoder"):
            ...
            try:
                ...
                pass
        text = text["text"]
        if not getattr(self.text_encoder, "has_llm", False):
            raise RuntimeError("文本编码器缺少 Qwen，请重新加载模型。")
    ...
    def generate(
        ...
        if not self.uncondition_mode:
            if hidden_state_dict is None:
                if progress_callback is not None and get_module_device(self).type == "cpu":
                    print(
                        f">>> Text encoding (CLIP + Qwen), then {self.validation_steps} diffusion steps on CPU…",
                        flush=True,
                    )
        ...
        with torch.no_grad():
            # Manual Euler when progress_callback is set (same as method=euler), so UI can report steps.
            method = (self._noise_scheduler_cfg or {}).get("method", "euler")
            if progress_callback is not None and method == "euler":
                y = y0
                for i in range(self.validation_steps):
                    dt = t[i + 1] - t[i]
                    y = y + fn(t[i], y) * dt
                    progress_callback(i + 1, self.validation_steps)
                sampled = y[:, :length, ...].clone()
            else:
                trajectory = odeint(fn, y0, t, **self._noise_scheduler_cfg)
                sampled = trajectory[-1][:, :length, ...].clone()
                if progress_callback is not None:
                    progress_callback(self.validation_steps, self.validation_steps)
