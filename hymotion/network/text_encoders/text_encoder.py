...
class HYTextModel(nn.Module):
    def __init__(
        ...
        if sentence_emb_type is not None:
            ...
            # HY_MOTION_DEVICE=cpu 时强制文本编码器落在 CPU，避免 Windows 上误用 GPU。
            # 权重保持 bfloat16（与磁盘一致），勿转 float32：否则 Qwen3-8B 约占 30GB 内存并极慢。
            _force_cpu = os.environ.get("HY_MOTION_DEVICE", "").lower() == "cpu"
            ...
            if _force_cpu:
                self.sentence_emb_text_encoder = self.sentence_emb_text_encoder.to("cpu")
        ...
        if llm_type is not None:
            ...
            if _force_cpu:
                # 内存不足时磁盘卸载，避免 Windows OOM 杀进程
                offload_dir = os.path.abspath(
                    os.path.join(LLM_ENCODER_LAYOUT[llm_type]["module_path"], "..", ".offload_qwen3")
                )
                os.makedirs(offload_dir, exist_ok=True)
                cpu_budget = os.environ.get("HY_MOTION_CPU_MEM", "6GiB")
                _llm_kwargs.update(
                    {
                        "device_map": "auto",
                ...
                print(f">>> Qwen CPU offload: max_memory={cpu_budget}, folder={offload_dir}")
            self.llm_text_encoder = LLM_ENCODER_LAYOUT[llm_type]["text_encoder_class"].from_pretrained(
                LLM_ENCODER_LAYOUT[llm_type]["module_path"],
                **_llm_kwargs,
            )
            if _force_cpu and not getattr(self.llm_text_encoder, "hf_device_map", None):
                self.llm_text_encoder = self.llm_text_encoder.to("cpu")
            self.llm_text_encoder = self.llm_text_encoder.eval().requires_grad_(False)
            print(">>> Text encoder Qwen3-8B loaded.")
    ...
    def to(self, *args, **kwargs):
        """避免对 accelerate/meta 权重做整体 .to()。"""
        llm = self.llm_text_encoder
        if llm is not None and getattr(llm, "hf_device_map", None) is not None:
            if self.sentence_emb_text_encoder is not None:
                self.sentence_emb_text_encoder = self.sentence_emb_text_encoder.to(*args, **kwargs)
            return self
        try:
    ...
    def encode_llm(self, text: List[str]) -> Tuple[Tensor, Tensor]:
        ...
        if device.type == "cpu":
            print(
                ">>> Qwen text encode on CPU (disk offload, may take 5–30+ min, please wait)…",
                flush=True,
            )
        llm_outputs = (
        ...
        if device.type == "cpu":
            print(">>> Qwen text encode done.", flush=True)
        if self.llm_type == "qwen3":
    ...
    def encode(self, text: List[str]) -> Tuple[Tensor, Tensor, Tensor]:
        if self.llm_type is None or self.llm_text_encoder is None:
            raise RuntimeError("Qwen 文本编码器未加载，无法编码文本。")
    ...
    def has_llm(self) -> bool:
        return self.llm_type is not None and self.llm_text_encoder is not None
